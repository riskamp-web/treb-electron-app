...
const { app, protocol, BrowserWindow, session, ipcMain, nativeTheme, Menu } = require("electron");
...
async function createWindow() {
  ...
  menuBuilder = MenuBuilder(win, app.name, (callback) => {
    win.webContents.send('menu', callback);
  });
...
// handle menu update
//
ipcMain.handle('menu', async (event, data) => {
  menuBuilder.UpdateMenu(data);
});
    ...
    case 'open':
      if (argument.path) {
        ...
        if (!result.canceled && result.filePaths.length) {
          ...
          const data = await fsx.readFile(file_path, "utf8");
          return { path: file_path, data, name: path.basename(file_path) };
        }
      }
      break;
